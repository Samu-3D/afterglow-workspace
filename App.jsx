...
function NewTaskModal({ space, onSave, onClose }) {
  ...
  return (
    <div style={{ position:"fixed", inset:0, background:"#000a", zIndex:100, display:"flex", alignItems:"center", justifyContent:"center" }} onClick={onClose}>
      <div onClick={e => e.stopPropagation()} style={{ background:C.surface, borderRadius:16, padding:"clamp(16px, 3vw, 28px)", width:500, maxWidth:"calc(100vw - 24px)", maxHeight:"90vh", overflowY:"auto", border:"1px solid "+C.border }}>
    ...
    <div style={{ ...PNL, minWidth:0, borderLeft:"4px solid "+C.gold }}>
      ...
      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))", gap:12 }}>
        ...
        <div style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:12, padding:12 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(150px, 1fr))", gap:8 }}><Input label="TARGET" type="number" value={purchaseForm.targetAmount} onChange={e => setPurchaseForm(f => ({ ...f, targetAmount:e.target.value }))} /><Input label="SAVED" type="number" value={purchaseForm.savedAmount} onChange={e => setPurchaseForm(f => ({ ...f, savedAmount:e.target.value }))} /></div>
        ...
        <div style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:12, padding:12 }}>
          ...
          <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(150px, 1fr))", gap:8 }}><Input label="TARGET" type="number" value={futureForm.targetAmount} onChange={e => setFutureForm(f => ({ ...f, targetAmount:e.target.value }))} /><Input label="SAVED" type="number" value={futureForm.savedAmount} onChange={e => setFutureForm(f => ({ ...f, savedAmount:e.target.value }))} /></div>
      ...
      {showMoreMoney && (
        <>
          ...
          {(moneyMode === "overview" || moneyMode === "ledger") && (
            <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(min(100%, 320px), 1fr))", gap:12, alignItems:"start", marginBottom:12 }}>
              <div style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:12, padding:12 }}>
    ...
    <div style={{ display:"grid", gridTemplateColumns:"minmax(0, 1fr)", gap:16 }}>
      <div style={PNL}>
        ...
        <div style={{ overflowX:"auto", paddingBottom:4 }}>
          <div style={{ display:"grid", gridTemplateColumns:"repeat(7, minmax(76px, 1fr))", gap:8, minWidth:"min(780px, 100%)" }}>
            {weekDays.map(d => (
            ...
            {cells.map((day, i) => {
              if (!day) {
                return <div key={i} style={{ minHeight:102, borderRadius:10, background:"#00000018", border:"1px dashed "+C.border, opacity:.45 }} />;
              }
...
function AuthGate({ backendNotice, backendBusy, onLogin, onRegister }) {
  ...
  const [authWidth, setAuthWidth] = useState(() => typeof window !== "undefined" ? window.innerWidth : 1366);
  const isRegister = mode === "register";
  const isAuthMobile = authWidth < 820;

  useEffect(() => {
    const onResize = () => setAuthWidth(window.innerWidth || 1366);
  ...
  return (
    <div style={{ minHeight:"100vh", background:`radial-gradient(circle at top left, ${C.orange}22, transparent 36%), radial-gradient(circle at bottom right, ${C.gold}18, transparent 34%), ${C.bg}`, color:C.cream, display:"flex", alignItems:isAuthMobile ? "flex-start" : "center", justifyContent:"center", padding:isAuthMobile ? 12 : 24, fontFamily:"Segoe UI, Helvetica Neue, sans-serif", overflowY:"auto", boxSizing:"border-box" }}>
      <div style={{ width:"100%", maxWidth:1040, display:"grid", gridTemplateColumns:isAuthMobile ? "1fr" : "minmax(0, 1.1fr) minmax(360px, .9fr)", gap:isAuthMobile ? 12 : 22, alignItems:"stretch" }}>
        <div style={{ background:C.surface, border:"1px solid "+C.border, borderRadius:isAuthMobile ? 16 : 22, padding:isAuthMobile ? 16 : 28, boxShadow:"0 28px 90px #0008", display:"flex", flexDirection:"column", justifyContent:"space-between", minHeight:isAuthMobile ? "auto" : 520 }}>
          <div>
            <Logo size="large" />
            <div style={{ color:C.gold, fontSize:12, letterSpacing:2, fontWeight:900, marginTop:20 }}>AFTERGLOW / MOPAS WORKSPACE</div>
            <h1 style={{ margin:"10px 0 10px", fontSize:isAuthMobile ? 28 : 42, lineHeight:1.05 }}>Sign in to continue your discipline system.</h1>
        ...
        <div style={{ background:C.surface, border:"1px solid "+C.border, borderRadius:isAuthMobile ? 16 : 22, padding:isAuthMobile ? 16 : 24, boxShadow:"0 28px 90px #0008", display:"flex", flexDirection:"column", justifyContent:"center" }}>
...
function AfterglowApp() {
  ...
  const isPhoneLayout = screenWidth < 760;
  const isTinyLayout = screenWidth < 480;
  const sidebarWidth = isPhoneLayout ? 236 : 260;
  const contentPadding = isPhoneLayout ? 10 : isCompactLayout ? 14 : 20;
  const headerPadding = isPhoneLayout ? "10px 12px" : "14px 24px";
  ...
  return (
    <div style={{ display:"flex", height:"100vh", background:C.bg, color:C.cream, fontFamily:"Segoe UI, Helvetica Neue, sans-serif", overflow:"hidden", fontSize: safeSettings.appearance.fontSize === "small" ? 13 : safeSettings.appearance.fontSize === "large" ? 16 : safeSettings.appearance.compactMode ? 13 : 14, position:"relative" }}>
      {isPhoneLayout && sidebarOpen && <div onClick={() => setSidebarOpen(false)} style={{ position:"fixed", inset:0, background:"#0009", zIndex:50 }} />}
      <aside style={{ width: sidebarOpen ? sidebarWidth : 0, minWidth: sidebarOpen ? sidebarWidth : 0, maxWidth:"86vw", background:C.surface, borderRight:"1px solid "+C.border, display:"flex", flexDirection:"column", transition:"all .2s", overflow:"hidden", ...(isPhoneLayout ? { position:"fixed", inset:"0 auto 0 0", height:"100vh", zIndex:60, boxShadow:sidebarOpen ? "18px 0 60px #000b" : "none" } : {}) }}>
      ...
      <main style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0 }}>
        <header style={{ padding:headerPadding, borderBottom:"1px solid "+C.border, display:"flex", justifyContent:"space-between", alignItems:"center", background:C.surface, flexWrap:"wrap", gap:10 }}>
          ...
          <div style={{ display:"flex", gap:8, alignItems:"center", flexWrap:"wrap", width:isPhoneLayout ? "100%" : "auto", overflowX:isPhoneLayout ? "auto" : "visible", paddingBottom:isPhoneLayout ? 2 : 0 }}>
        ...
        {view !== "dashboard" && view !== "settings" && (
          <div style={{ padding:isPhoneLayout ? "10px 12px" : "12px 24px", borderBottom:"1px solid "+C.border, background:C.surface, display:"grid", gridTemplateColumns:`repeat(auto-fit, minmax(${isPhoneLayout ? 130 : 150}px, 1fr))`, gap:isPhoneLayout ? 8 : 10, alignItems:"center" }}>
        ...
        <div style={{ flex:1, overflow:"auto", padding:contentPadding, minWidth:0, WebkitOverflowScrolling:"touch" }}>
          {view === "dashboard" ? <Dashboard tasks={tasks} activeSpace={activeSpace} settings={safeSettings} goSpace={goSpace} setView={setView} setActiveSpace={setActiveSpace} setSelected={setSelected} setShowNewTask={setShowNewTask} setShowEndDayReview={setShowEndDayReview} onUpdate={updateTask} /> : view === "settings" ? (
            ...
            <div style={{ display:"grid", gridTemplateColumns: (view === "list" || view === "board") ? (isCompactLayout ? "1fr" : "minmax(0, 1fr) minmax(320px, 500px)") : "1fr", gap:isPhoneLayout ? 12 : 18, alignItems:"start", minWidth:0, maxWidth:"100%" }}>
