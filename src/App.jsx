...
const persistTasksNow = (items) => {
  const safeItems = Array.isArray(items) ? items : [];
  try {
    const normalized = safeItems.map(item => {
      try { return typeof normalizeTask === "function" ? normalizeTask(item) : item; } catch { return item; }
    });
    save(normalized);
  } catch {
    save(safeItems);
...
const ACTIVE_SPACE_KEY = "afterglow_active_space_v1";
const TASK_SCOPE_FILTER_KEY = "afterglow_task_scope_filter_v1";
...
const DEFAULT_APP_SETTINGS = {
  ...
  commandCenter:{
    ...
    title:"AFTERGLOW Command Center",
    intro:"One calm control board for today: routine, tasks, MOPAS risks, documents, money, and creative progress.",
    coachLabel:"Mental Coach",
    showQuickChips:true,
    focusMode:"balanced",
  },
  ...
  appearance:{ theme:"dark", accentColor:"orange", compactMode:false, cardDensity:"comfortable", sidebarBehavior:"auto", logoSize:"medium", fontSize:"normal", borderRadius:"rounded", panelSpacing:"normal", dashboardIcon:"▨", dashboardLabel:"Command Center", workspaceCard:true, showTaskCounts:true, showTopSettingsButton:false, showTopQuickActions:false, showCloudSyncPanel:true, showLoginDetails:false },
  notifications:{ browserNotifications:false, phoneTaskReminders:true, reminderMinutesBefore:10, reminderRepeatMinutes:30, reminderScope:"Today + overdue", quietHoursStart:"23:00", quietHoursEnd:"05:30", vibrateOnReminder:true, emailNotifications:true, todayDisciplineEmail:true, todayDisciplineEmailTime:"05:45", emailMode:"Current action only", emailTone:"Direct", actionWindowMinutes:60, maxEmailTasks:1, includeTaskStartTimes:true, includeWhyInEmail:true, includeNextBlock:true, includeLateWarningInCurrentEmail:true, includeDocumentAlertInCurrentEmail:false, includeRoutineProgressInCurrentEmail:true, dailyMorningPlanEmail:true, overdueEmail:true, documentExpiryEmail:true, endDayReviewEmail:true, dailyRoutineReminder:true, deadlineReminder:true, tenderDeadlineReminder:true, whatsappNumber:"+250784623361", emailAddress:"ishimwesamuel3d@gmail.com", method:"Phone reminder + Email", googleAppsScriptEmailUrl:"" },
};
const mergeAppSettings = (value = {}) => {
  ...
  return {
    ...
    commandCenter:{
      ...
      showQuickChips: rawCommandCenter.showQuickChips !== false,
      focusMode: ["minimal", "balanced", "executive"].includes(rawCommandCenter.focusMode) ? rawCommandCenter.focusMode : DEFAULT_APP_SETTINGS.commandCenter.focusMode,
      title: rawCommandCenter.title || DEFAULT_APP_SETTINGS.commandCenter.title,
      intro: rawCommandCenter.intro || DEFAULT_APP_SETTINGS.commandCenter.intro,
      coachLabel: rawCommandCenter.coachLabel || DEFAULT_APP_SETTINGS.commandCenter.coachLabel,
      ...
      <div style={{ ...PNL, minWidth:0, borderLeft:"5px solid "+C.orange, padding:22, background:`linear-gradient(135deg, ${C.surface}, ${C.elevated})`, boxShadow:"0 16px 45px #0004" }}>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))", gap:14, alignItems:"start" }}>
          <div style={{ minWidth:0 }}>
            ...
            <h2 style={{ margin:"0 0 6px", color:C.cream, fontSize:24, lineHeight:1.15 }}>{mergedSettings.commandCenter.title || mergedSettings.appearance.dashboardLabel || "Command Center"}</h2>
            <div style={{ color:C.creamSoft, fontSize:13, lineHeight:1.5 }}>{mergedSettings.commandCenter.intro || "Calendar is preserved. Financial Health now lives inside Money & Savings. Choose sections in Settings, arrange their order, and use Show More when you need the full control board."}</div>
          </div>
...
function SettingsView({ settings, setSettings, tasks, exportBackup, importBackup, resetSettingsOnly, clearTestData, startFreshToday, sendTodayDisciplineEmail, emailNotice }) {
  ...
  const settingsSearchText = {
    ...
    "Tasks & Deadlines":"priority deadline overdue late done completed reminder days week start",
    "Command Center":"dashboard command center sections calendar show more layout density presets coach",
    "MOPAS Tender":"mopas tender root folder documents stages warning threshold urgency",
    "Documents":"document categories expiry drive folder owner apps script upload",
    "Appearance":"theme color font density sidebar logo dashboard icon top actions settings button",
    "Notifications":"phone reminder browser email vibration quiet hours action email apps script whatsapp",
    "Backup & Data":"backup import export json reset local storage test data"
  };
  const settingsMatches = (name) => {
    const q = settingsQuery.trim().toLowerCase();
    if (!q) return true;
    return `${name} ${settingsSearchText[name] || ""}`.toLowerCase().includes(q);
  };
  return (
    <div style={{ display:"grid", gap:16, minWidth:0 }}>
      ...
      <div style={{ ...PNL, padding:12, display:"grid", gap:10 }}>
        ...
        {settingsQuery.trim() && <div style={{ display:"flex", gap:6, flexWrap:"wrap" }}>
          {tabs.filter(settingsMatches).map(name => <Btn key={name} small orange={tab === name} ghost={tab !== name} onClick={() => setTab(name)}>{name}</Btn>)}
          {!tabs.filter(settingsMatches).length && <span style={{ color:C.muted, fontSize:12 }}>No exact tab match. Try “notification”, “appearance”, “command”, “backup”, or “name”.</span>}
        </div>}
      </div>
      <div style={{ display:"flex", gap:8, flexWrap:"wrap" }}>
      ...
      {tab === "Platform" && <SettingCard title="PLATFORM SETTINGS" note="Control the whole platform from one place: identity, login text, phone layout, sync visibility, and daily start behavior.">
        <FieldGrid>
          ...
          <Input label="START NOTE" value={merged.general.startNote || "Ready to run your day."} onChange={e => update("general", "startNote", e.target.value)} />
          <Input label="SUPPORT NOTE" value={merged.general.supportNote || "Built for phone, tablet, and desktop use."} onChange={e => update("general", "supportNote", e.target.value)} />
          <Select label="DEFAULT SPACE AFTER LOGIN" options={SPACES.map(s => s.id)} value={merged.general.defaultSpace} onChange={e => update("general", "defaultSpace", e.target.value)} />
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(230px, 1fr))", gap:8, marginTop:12 }}>
          <Toggle label="Show account/sync panel in sidebar" checked={merged.appearance.showCloudSyncPanel !== false} onChange={v => update("appearance", "showCloudSyncPanel", v)} />
          <Toggle label="Show top quick actions" checked={merged.appearance.showTopQuickActions === true} onChange={v => update("appearance", "showTopQuickActions", v)} />
      ...
      {tab === "Brand & Login" && <SettingCard title="BRAND & LOGIN" note="Make the entry screen cleaner or more personal without touching code.">
        <FieldGrid>
          ...
          <Input label="LOGIN SUBTITLE" value={merged.general.loginSubtitle || "Your daily operating system."} onChange={e => update("general", "loginSubtitle", e.target.value)} />
        ...
        <div style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:12, padding:14, marginTop:12, display:"flex", alignItems:"center", gap:14 }}>
          <Logo size={merged.appearance.logoSize || "medium"} />
          <div><div style={{ color:C.cream, fontWeight:900 }}>{merged.general.loginTitle}</div><div style={{ color:C.creamSoft, fontSize:12, marginTop:4 }}>{merged.general.loginSubtitle}</div></div>
        </div>
      </SettingCard>}
      ...
      {tab === "Command Center" && <SettingCard title="COMMAND CENTER" note="Build your own dashboard: keep Calendar visible, choose sections, arrange order, and control how much appears before Show More. Financial Health is managed inside Money & Savings.">
        <FieldGrid>
          <Input label="COMMAND CENTER TITLE" value={merged.commandCenter.title || "AFTERGLOW Command Center"} onChange={e => update("commandCenter", "title", e.target.value)} />
          <Input label="COMMAND CENTER INTRO" value={merged.commandCenter.intro || ""} onChange={e => update("commandCenter", "intro", e.target.value)} />
          <Input label="COACH PANEL LABEL" value={merged.commandCenter.coachLabel || "Mental Coach"} onChange={e => update("commandCenter", "coachLabel", e.target.value)} />
        ...
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(220px, 1fr))", gap:10, marginTop:8 }}>
          ...
          <Toggle label="Show quick chips / metrics" checked={merged.commandCenter.showQuickChips !== false} onChange={v => update("commandCenter", "showQuickChips", v)} />
        </div>
      ...
      {tab === "Notifications" && <SettingCard title="PHONE REMINDERS & EMAIL" note="Use phone/browser reminders for active work, and email for scheduled summaries.">
        ...
        <FieldGrid>
          <Select label="REMINDER METHOD" options={["Phone reminder + Email", "Phone reminder", "Email", "Off"]} value={merged.notifications.method || "Phone reminder + Email"} onChange={e => update("notifications", "method", e.target.value)} />
          <Select label="REMINDER SCOPE" options={["Today + overdue", "Today only", "High priority only", "All open tasks"]} value={merged.notifications.reminderScope || "Today + overdue"} onChange={e => update("notifications", "reminderScope", e.target.value)} />
          <Input label="REMIND BEFORE START (minutes)" type="number" value={merged.notifications.reminderMinutesBefore || 10} onChange={e => update("notifications", "reminderMinutesBefore", Number(e.target.value || 10))} />
          <Input label="REPEAT EVERY (minutes)" type="number" value={merged.notifications.reminderRepeatMinutes || 30} onChange={e => update("notifications", "reminderRepeatMinutes", Number(e.target.value || 30))} />
          ...
          <Input label="QUIET HOURS END" type="time" value={merged.notifications.quietHoursEnd || "05:30"} onChange={e => update("notifications", "quietHoursEnd", e.target.value)} />
        </FieldGrid>
        <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit, minmax(240px, 1fr))", gap:7, margin:"10px 0 14px" }}>
          <Toggle label="Enable phone/browser task reminders" checked={merged.notifications.phoneTaskReminders !== false} onChange={v => update("notifications", "phoneTaskReminders", v)} />
          <Toggle label="Enable vibration when supported" checked={merged.notifications.vibrateOnReminder !== false} onChange={v => update("notifications", "vibrateOnReminder", v)} />
          <Toggle label="Enable browser notifications" checked={merged.notifications.browserNotifications === true} onChange={v => update("notifications", "browserNotifications", v)} />
        </div>
        <div style={{ background:C.bg, border:"1px solid "+C.border, borderLeft:"4px solid "+C.orange, borderRadius:12, padding:14, marginBottom:16 }}>
          <div style={{ color:C.orange, fontSize:10, letterSpacing:2, fontWeight:900, marginBottom:6 }}>EMAIL MODE</div>
          <div style={{ color:C.creamSoft, fontSize:12, lineHeight:1.65 }}>
            Email sends a focused action summary through Google Apps Script. Without the Apps Script URL, phone reminders and the app still work.
          </div>
        </div>
        <FieldGrid>
...
function AuthGate({ backendNotice, backendBusy, onLogin, onRegister, settings }) {
  ...
  const [form, setForm] = useState({ name:merged.general.userName || "Samu", email:"ishimwesamuel3d@gmail.com", password:"" });
  const submit = () => {
    const payload = {
      name: form.name.trim() || merged.general.userName || "Samu",
      ...
      <div style={{ width:"100%", maxWidth:460, display:"grid", gap:14, alignItems:"stretch" }}>
        <div style={{ background:C.surface, border:"1px solid "+C.border, borderRadius:22, padding:"clamp(16px, 5vw, 26px)", boxShadow:"0 26px 90px #0008", boxSizing:"border-box" }}>
          ...
          <div style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:18, padding:16 }}>
            ...
            <div style={{ color:C.gold, fontSize:11, letterSpacing:2, fontWeight:900, marginBottom:10 }}>{mode === "register" ? "CREATE ACCOUNT" : "SIGN IN"}</div>
            ...
            <Btn orange disabled={backendBusy || !form.email || !form.password} onClick={submit} style={{ width:"100%", padding:"12px 18px", marginTop:4 }}>{backendBusy ? "Opening workspace..." : mode === "register" ? "Create Account" : "Login"}</Btn>
            {visibleNotice && (
              <div style={{ marginTop:14, background:C.red+"18", border:"1px solid "+C.red, color:C.cream, borderRadius:12, padding:12, fontSize:12, lineHeight:1.45 }}>
                {visibleNotice.message}
              </div>
            )}
          </div>
          <div style={{ color:C.muted, fontSize:11, marginTop:14, lineHeight:1.5 }}>{merged.general.startNote || "Ready to run your day."}</div>
        </div>
      </div>
    </div>
  );
        ...
        <div style={{ ...popupPosition, background:C.surface, border:"1px solid "+C.border, borderRadius:14, padding:14, zIndex:80, boxShadow:"0 20px 60px #0009" }}>
          <div style={{ display:"flex", justifyContent:"space-between", gap:8, alignItems:"center", marginBottom:10 }}>
            <div>
              ...
              <div style={{ color:C.creamSoft, fontSize:12 }}>{isConnected ? "Signed in and ready" : "Sign in to sync"}</div>
            </div>
          ...
          {isConnected ? (
            <>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
                <Btn small orange disabled={backendBusy} onClick={onRefresh}>Refresh</Btn>
                <Btn small disabled={backendBusy} onClick={onUploadLocal}>Upload</Btn>
            ...
            <>
              ...
              <Input label="EMAIL" type="email" value={form.email} onChange={e => setForm(f => ({ ...f, email:e.target.value }))} />
              <Input label="PASSWORD" type="password" value={form.password} onChange={e => setForm(f => ({ ...f, password:e.target.value }))} placeholder="Your password" />
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:8 }}>
          ...
          {visibleNotice && (
            <div style={{ marginTop:10, background:C.red+"18", border:"1px solid "+C.red, color:C.cream, borderRadius:10, padding:10, fontSize:12, lineHeight:1.4 }}>
              {visibleNotice.message}
            </div>
          )}
        </div>
...
function LifeOSFutureView({ tasks = [], settings, setActiveSpace, setView, setSelected, setShowNewTask, onUpdate, isPhoneLayout }) {
  const [suiteTab, setSuiteTab] = useState("command");
  ...
  const allFocusCandidates = [...todayTasks, ...lateTasks, ...urgentTasks, ...openTasks];
  const topAction = sortTasksSmart(allFocusCandidates)[0] || null;
  ...
  const moneyNet = monthlyIncome - monthlyExpense;
  const lifeScore = Math.min(100, Math.round(
    (routinePct * .28) +
    (Math.min(100, doneToday * 16) * .18) +
    ((lateTasks.length ? Math.max(0, 100 - lateTasks.length * 12) : 100) * .20) +
    ((itsindaThisWeek >= ITSINDA_WEEKLY_AMOUNT ? 100 : Math.round((itsindaThisWeek / ITSINDA_WEEKLY_AMOUNT) * 100)) * .14) +
    ((riskDocs.length ? Math.max(0, 100 - riskDocs.length * 16) : 100) * .10) +
    ((tenderTasks.length || normalMopasTasks.length ? Math.min(100, (tenderTasks.length + normalMopasTasks.length) * 10) : 50) * .10)
  ));
  const now = new Date();
  const nowMinutes = now.getHours() * 60 + now.getMinutes();
  const timeToMinutes = (time) => {
  ...
  const currentBlock = TODAY_DISCIPLINE_BLOCKS.find(block => nowMinutes >= timeToMinutes(block.start) && nowMinutes < timeToMinutes(block.end));
  const nextBlock = TODAY_DISCIPLINE_BLOCKS.find(block => timeToMinutes(block.start) > nowMinutes);
  const scoreColor = lifeScore >= 75 ? C.green : lifeScore >= 50 ? C.gold : C.red;
  const gridTwo = isPhoneLayout ? "1fr" : "repeat(auto-fit, minmax(290px, 1fr))";
  const gridThree = isPhoneLayout ? "1fr" : "repeat(auto-fit, minmax(220px, 1fr))";
  const compactStats = isPhoneLayout ? "repeat(2, minmax(0, 1fr))" : "repeat(auto-fit, minmax(160px, 1fr))";
  ...
  const startTopAction = () => {
    if (!topAction) return;
    onUpdate({ ...topAction, status:"In Progress" });
    openTask({ ...topAction, status:"In Progress" });
  };
  const recoverLateTask = () => {
    const task = lateTasks[0];
    if (!task) return;
    onUpdate({ ...task, status:"In Progress", priority:task.priority === "Low" ? "Normal" : task.priority });
    openTask({ ...task, status:"In Progress" });
  };
  ...
  const boardTasks = sortTasksSmart(
    boardScope === "all" ? safeTasks :
    boardScope === "tenders" ? safeTasks.filter(isMopasTenderWork) :
    boardScope === "mopas" ? safeTasks.filter(t => t.space === "mopas") :
    safeTasks.filter(t => t.space === boardScope)
  );
  const boardColumns = STATUSES.map(status => ({ status, tasks:boardTasks.filter(t => (t.status || "To Do") === status) }));
  const boardScopes = [
    { id:"all", label:"All Life" },
    { id:"mopas", label:"MOPAS" },
    { id:"tenders", label:"Tenders" },
    { id:"money", label:"Money" },
    { id:"drawing", label:"Creative" },
    { id:"health", label:"Health" },
    { id:"afterglow", label:"Brand" },
  ];
  const healthMap = [
    { label:"Command clarity", value:topAction ? 100 : 40, note:topAction ? topAction.title : "No next action selected", color:topAction ? C.green : C.orange },
    { label:"Routine discipline", value:routinePct, note:`${routineDone}/${routineToday.length || DAILY_ROUTINES.length} routines done`, color:routinePct >= 70 ? C.green : C.gold },
    { label:"Late recovery", value:lateTasks.length ? Math.max(0, 100 - lateTasks.length * 12) : 100, note:lateTasks.length ? `${lateTasks.length} late tasks` : "No late risk", color:lateTasks.length ? C.red : C.green },
    { label:"Money control", value:monthlyIncome ? Math.max(0, Math.min(100, Math.round(((monthlyIncome - monthlyExpense + monthlySavings) / monthlyIncome) * 100))) : 50, note:`Month net ${rwf(moneyNet)}`, color:moneyNet >= 0 ? C.green : C.red },
    { label:"Document safety", value:riskDocs.length ? Math.max(0, 100 - riskDocs.length * 16) : 100, note:riskDocs.length ? `${riskDocs.length} document alerts` : "Documents safe", color:riskDocs.length ? C.red : C.green },
  ...
  const automationRules = [
    ...
    { title:"ITSINDA savings watch", trigger:"Every week", action:"Compare saved money with 20,000 RWF weekly target", status:itsindaThisWeek >= ITSINDA_WEEKLY_AMOUNT ? "On track" : "Pending", color:itsindaThisWeek >= ITSINDA_WEEKLY_AMOUNT ? C.green : C.gold },
    { title:"Tender pipeline intelligence", trigger:"MOPAS tender task created", action:"Classify as tender work or normal MOPAS task and place it in the right system", status:"Active", color:C.blue },
  ];
  const knowledgeTemplates = [
    { title:"Tender preparation template", tag:"MOPAS", body:"Tender summary\nDeadline\nRequired documents\nMissing documents\nTechnical proposal\nFinancial offer\nSubmission proof\nFollow-up owner" },
    { title:"Daily operation report", tag:"Report", body:"Completed today\nPending work\nTomorrow priority\nMOPAS follow-ups\nRisks / support needed\nNotes" },
    { title:"Creative project note", tag:"AFTERGLOW", body:"Project idea\nReference style\nStory point\nVisual test\nNext production action\nDeadline" },
    { title:"Money decision note", tag:"Money", body:"Income today\nExpenses today\nSaving action\nPlanned purchase\nDebt / pending payment\nTomorrow money decision" },
  ];
  const roadmap = [
    { phase:"Now", title:"All-in-one command suite", status:"Built in this version", color:C.green, items:["Command cockpit", "Boards", "Data hub", "Automations", "Reports", "Knowledge", "Roadmap"] },
    { phase:"Next", title:"Real backend databases", status:"Frontend ready", color:C.gold, items:["Tenders API", "Documents API", "Money API", "Reports API", "Clients API"] },
    { phase:"After", title:"Team collaboration", status:"Future", color:C.blue, items:["Owners", "Comments", "File attachment", "Approval flow", "Client follow-up"] },
    { phase:"Power", title:"AI assistant inside app", status:"Future", color:C.purple, items:["Daily advice", "Tender checklist review", "Boss report draft", "Money warning", "Creative coach"] },
  ];
  ...
  const MetricCard = ({ label, value, color = C.orange, sub }) => (
    <div style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:14, padding:isPhoneLayout ? 10 : 14, minWidth:0 }}>
      <div style={{ color:C.creamSoft, fontSize:10, letterSpacing:1.5, fontWeight:900, textTransform:"uppercase" }}>{label}</div>
      <div style={{ color, fontSize:isPhoneLayout ? 20 : 26, fontWeight:900, marginTop:4, lineHeight:1, wordBreak:"break-word" }}>{value}</div>
      {sub && <div style={{ color:C.muted, fontSize:11, marginTop:7, lineHeight:1.35 }}>{sub}</div>}
    </div>
  );
  const TaskMini = ({ task, compact = false }) => {
    const late = taskLateInfo(task);
    return (
      <div onClick={() => openTask(task)} style={{ background:C.bg, border:"1px solid "+(late.isLate ? C.red : C.border), borderLeft:"4px solid "+(late.isLate ? C.red : priorityColor(task.priority)), borderRadius:12, padding:compact ? 9 : 11, cursor:"pointer", marginBottom:8, minWidth:0 }}>
        <div style={{ display:"flex", justifyContent:"space-between", gap:8, alignItems:"flex-start" }}>
          <div style={{ minWidth:0, flex:1 }}>
            <div style={{ color:late.isLate ? C.red : C.cream, fontWeight:900, fontSize:compact ? 12 : 13, lineHeight:1.35, wordBreak:"break-word" }}>{task.title}</div>
  ...
  const SectionTitle = ({ eyebrow, title, action, sub }) => (
    <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", gap:10, flexWrap:"wrap", marginBottom:12 }}>
      <div>
        ...
        {sub && <div style={{ color:C.creamSoft, fontSize:12, marginTop:4, lineHeight:1.45 }}>{sub}</div>}
      </div>
      {action}
    </div>
  );
  const Panel = ({ children, style }) => <div style={{ ...PNL, padding:isPhoneLayout ? 14 : 18, minWidth:0, ...style }}>{children}</div>;
  ...
  return (
    <div style={{ display:"grid", gap:isPhoneLayout ? 12 : 16, maxWidth:1540, margin:"0 auto", width:"100%" }}>
      <div style={{ ...PNL, padding:isPhoneLayout ? 14 : 22, background:`linear-gradient(135deg, ${C.surface}, ${C.bg})`, overflow:"hidden" }}>
        <div style={{ display:"grid", gridTemplateColumns:isPhoneLayout ? "1fr" : "minmax(0, 1.45fr) minmax(260px, .55fr)", gap:16, alignItems:"center" }}>
          <div style={{ minWidth:0 }}>
            <div style={{ color:C.gold, fontSize:11, letterSpacing:2, fontWeight:900 }}>AFTERGLOW LIFE OS COMMAND SUITE</div>
            <h1 style={{ margin:"6px 0 8px", color:C.cream, fontSize:isPhoneLayout ? 24 : 36, lineHeight:1.05 }}>One control room for tasks, tenders, documents, money, reports, knowledge, and discipline.</h1>
            <p style={{ margin:0, color:C.creamSoft, lineHeight:1.6, fontSize:isPhoneLayout ? 13 : 14 }}>This version changes the way the app works: instead of only opening separate sections, you now have a command suite with boards, databases, automations, reports, and a knowledge base connected to your real work.</p>
            <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:14 }}>
              <Btn orange onClick={startTopAction} disabled={!topAction}>{topAction ? "Start next mission" : "No mission now"}</Btn>
              <Btn ghost onClick={() => setShowNewTask(true)}>+ Capture task</Btn>
              <Btn ghost onClick={() => goSpaceView("mopas", "list")}>MOPAS workspace</Btn>
              <Btn ghost onClick={() => goSpaceView("money", "list")}>Money workspace</Btn>
            </div>
          </div>
          <div style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:18, padding:isPhoneLayout ? 14 : 18 }}>
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", gap:12, marginBottom:10 }}>
              <div>
                ...
                <div style={{ color:scoreColor, fontSize:42, lineHeight:1, fontWeight:900 }}>{lifeScore}%</div>
              </div>
              <Badge color={scoreColor}>{todayKey}</Badge>
            </div>
            <ProgressBar value={lifeScore} color={scoreColor} />
            <div style={{ color:C.muted, fontSize:11, marginTop:9, lineHeight:1.5 }}>Score combines routines, completed work, late risk, MOPAS movement, ITSINDA savings, and document safety.</div>
          </div>
        </div>
      </div>
      ...
      <div style={{ display:"flex", gap:8, overflowX:"auto", paddingBottom:2 }}>
        {tabs.map(tab => (
          <button key={tab.id} onClick={() => setSuiteTab(tab.id)} style={{ border:"1px solid "+(suiteTab === tab.id ? C.orange : C.border), background:suiteTab === tab.id ? C.orange+"22" : C.surface, color:suiteTab === tab.id ? C.orange : C.creamSoft, borderRadius:999, padding:"8px 13px", fontSize:12, fontWeight:900, cursor:"pointer", whiteSpace:"nowrap", flexShrink:0 }}>{tab.label}</button>
        ))}
      </div>
      ...
      {suiteTab === "command" && (
        <>
          <div style={{ display:"grid", gridTemplateColumns:compactStats, gap:10 }}>
            ...
            <MetricCard label="Month net" value={rwf(moneyNet)} color={moneyNet >= 0 ? C.green : C.red} sub={`${rwf(monthlySavings)} saved this month`} />
          </div>
          <div style={{ display:"grid", gridTemplateColumns:gridTwo, gap:14 }}>
            <Panel style={{ borderColor:topAction ? C.orange : C.border }}>
              <SectionTitle eyebrow="Now" title="Next best action" sub={currentBlock ? `Current block: ${currentBlock.start}-${currentBlock.end} · ${currentBlock.title}` : "Outside the fixed discipline blocks."} action={<Btn small orange onClick={startTopAction} disabled={!topAction}>Start</Btn>} />
              {topAction ? <TaskMini task={topAction} /> : <div style={{ color:C.creamSoft, fontSize:13 }}>No action found. Capture the next task or open a workspace.</div>}
              {nextBlock && <div style={{ marginTop:10, color:C.muted, fontSize:12 }}>Next block: <strong style={{ color:C.cream }}>{nextBlock.start}-{nextBlock.end}</strong> · {nextBlock.title}</div>}
            </Panel>
            <Panel>
              <SectionTitle eyebrow="Recovery" title="Late task control" action={<Btn small ghost onClick={recoverLateTask} disabled={!lateTasks.length}>Recover first</Btn>} />
              {lateTasks.slice(0, 4).map(task => <TaskMini key={task.id} task={task} compact />)}
              {!lateTasks.length && <div style={{ color:C.green, fontWeight:800 }}>Clean. No late task right now.</div>}
            </Panel>
          </div>
          <Panel>
            <SectionTitle eyebrow="Health map" title="System health" sub="This works like a dashboard: you see the weak area first, then act." />
            <div style={{ display:"grid", gridTemplateColumns:gridThree, gap:10 }}>
              {healthMap.map(item => (
                <div key={item.label} style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:12, padding:12 }}>
                  <div style={{ display:"flex", justifyContent:"space-between", gap:10, marginBottom:8 }}><strong style={{ color:C.cream, fontSize:13 }}>{item.label}</strong><span style={{ color:item.color, fontWeight:900 }}>{Math.round(item.value)}%</span></div>
                  <ProgressBar value={item.value} color={item.color} />
                  <div style={{ color:C.muted, fontSize:11, marginTop:7 }}>{item.note}</div>
                </div>
              ))}
            </div>
          </Panel>
          <Panel>
            <SectionTitle eyebrow="Workspaces" title="Your life as connected systems" />
            <div style={{ display:"grid", gridTemplateColumns:gridThree, gap:10 }}>
              {systemCards.map(card => (
                <div key={card.id} onClick={() => goSpaceView(card.id, "list")} style={{ background:C.bg, border:"1px solid "+(card.late ? C.red : C.border), borderTop:"4px solid "+card.color, borderRadius:14, padding:13, cursor:"pointer" }}>
                  <div style={{ display:"flex", justifyContent:"space-between", gap:8 }}><strong style={{ color:C.cream }}>{card.name}</strong><Badge color={card.late ? C.red : card.color}>{card.open} open</Badge></div>
                  <div style={{ display:"grid", gridTemplateColumns:"repeat(3, 1fr)", gap:6, marginTop:12, color:C.creamSoft, fontSize:11 }}>
                    <div>Today<br /><strong style={{ color:C.cream }}>{card.today}</strong></div>
                    <div>Late<br /><strong style={{ color:card.late ? C.red : C.green }}>{card.late}</strong></div>
                    <div>Done<br /><strong style={{ color:C.green }}>{card.done}</strong></div>
                  </div>
                  <div style={{ marginTop:10 }}><ProgressBar value={card.score} color={card.color} /></div>
                </div>
              ))}
            </div>
          </Panel>
      ...
      {suiteTab === "boards" && (
        <Panel>
          <SectionTitle eyebrow="Boards" title="Monday / Trello style board system" sub="Move work mentally from To Do to In Progress to Done. Click any card to open the real task detail." action={<Btn small orange onClick={() => setShowNewTask(true)}>+ New card</Btn>} />
          <div style={{ display:"flex", gap:8, overflowX:"auto", marginBottom:12 }}>
            {boardScopes.map(scope => <button key={scope.id} onClick={() => setBoardScope(scope.id)} style={{ border:"1px solid "+(boardScope === scope.id ? C.orange : C.border), background:boardScope === scope.id ? C.orange+"22" : C.bg, color:boardScope === scope.id ? C.orange : C.creamSoft, borderRadius:999, padding:"7px 12px", fontSize:12, fontWeight:900, cursor:"pointer", whiteSpace:"nowrap" }}>{scope.label}</button>)}
          </div>
          <div style={{ display:"grid", gridTemplateColumns:isPhoneLayout ? "1fr" : "repeat(4, minmax(220px, 1fr))", gap:10, alignItems:"start" }}>
            {boardColumns.map(col => (
              <div key={col.status} style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:14, padding:10, minHeight:120 }}>
                <div style={{ display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:10 }}><strong style={{ color:statusColor(col.status) }}>{col.status}</strong><Badge color={statusColor(col.status)}>{col.tasks.length}</Badge></div>
                {col.tasks.slice(0, 10).map(task => <TaskMini key={task.id} task={task} compact />)}
                {!col.tasks.length && <div style={{ color:C.muted, fontSize:12 }}>No cards</div>}
              </div>
            ))}
          </div>
        </Panel>
      ...
      {suiteTab === "data" && (
        <div style={{ display:"grid", gridTemplateColumns:gridTwo, gap:14 }}>
          <Panel>
            <SectionTitle eyebrow="Airtable style" title="Tender database" action={<Btn small ghost onClick={() => goSpaceView("mopas", "list")}>Open MOPAS</Btn>} />
            {pipelineStages.map(stage => (
              <div key={stage.stage} style={{ display:"flex", justifyContent:"space-between", alignItems:"center", background:C.bg, border:"1px solid "+C.border, borderRadius:10, padding:10, marginBottom:8 }}>
                <span style={{ color:C.cream, fontWeight:800 }}>{stage.stage}</span><Badge color={stage.tasks.length ? C.blue : C.muted}>{stage.tasks.length} items</Badge>
              </div>
            ))}
          </Panel>
          <Panel>
            <SectionTitle eyebrow="Document system" title="Risk database" action={<Btn small ghost onClick={() => goSpaceView("mopas", "documents")}>Open documents</Btn>} />
            {riskDocs.slice(0, 6).map(doc => (
              <div key={doc.id || doc.name} style={{ background:C.bg, border:"1px solid "+(doc.statusInfo.label === "Expired" ? C.red : C.orange), borderRadius:10, padding:10, marginBottom:8 }}>
                <div style={{ color:C.cream, fontWeight:800 }}>{doc.name || doc.title || "Document"}</div>
                <div style={{ color:C.muted, fontSize:12 }}>{doc.category || "Document"} · {doc.expiryDate || "No date"} · {doc.statusInfo.label}</div>
              </div>
            ))}
            {!riskDocs.length && <div style={{ color:C.green, fontWeight:800 }}>No document risk right now.</div>}
          </Panel>
          <Panel>
            <SectionTitle eyebrow="Money database" title="Financial control" action={<Btn small ghost onClick={() => goSpaceView("money", "list")}>Open money</Btn>} />
            <div style={{ display:"grid", gridTemplateColumns:"repeat(2, minmax(0, 1fr))", gap:8 }}>
              ...
              <MetricCard label="Purchase saved" value={rwf(totalPurchaseSaved)} color={C.gold} sub={totalPurchaseTarget ? `Target ${rwf(totalPurchaseTarget)}` : "Set targets"} />
              <MetricCard label="Future saved" value={rwf(totalFutureSaved)} color={C.purple} sub={totalFutureTarget ? `Target ${rwf(totalFutureTarget)}` : "Set targets"} />
            </div>
          </Panel>
          <Panel>
            <SectionTitle eyebrow="Goals database" title="Life goals + project goals" action={<Btn small ghost onClick={() => goSpaceView("drawing", "Goals")}>Open goals</Btn>} />
            {[...purchaseGoals.slice(0, 3), ...futureGoals.slice(0, 3)].map(goal => (
              <div key={goal.id || goal.title} style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:10, padding:10, marginBottom:8 }}>
                <div style={{ display:"flex", justifyContent:"space-between", gap:8 }}><strong style={{ color:C.cream }}>{goal.title}</strong><span style={{ color:C.gold, fontSize:12 }}>{moneyProgress(goal.savedAmount, goal.targetAmount)}%</span></div>
                <div style={{ marginTop:6 }}><ProgressBar value={moneyProgress(goal.savedAmount, goal.targetAmount)} color={C.gold} /></div>
              </div>
            ))}
          </Panel>
        </div>
      ...
      {suiteTab === "automations" && (
        <Panel>
          <SectionTitle eyebrow="Automation lab" title="Rules that make the app work for you" sub="This is the beginning of a real automation system. Some rules already run; others are shown as the next backend upgrade." />
          <div style={{ display:"grid", gridTemplateColumns:gridTwo, gap:10 }}>
            {automationRules.map(rule => (
              <div key={rule.title} style={{ background:C.bg, border:"1px solid "+rule.color, borderRadius:14, padding:13 }}>
                <div style={{ display:"flex", justifyContent:"space-between", gap:8, marginBottom:8 }}><strong style={{ color:C.cream }}>{rule.title}</strong><Badge color={rule.color}>{rule.status}</Badge></div>
                <div style={{ color:C.creamSoft, fontSize:12, lineHeight:1.5 }}><strong>Trigger:</strong> {rule.trigger}<br /><strong>Action:</strong> {rule.action}</div>
              </div>
            ))}
          </div>
          <div style={{ display:"flex", gap:8, flexWrap:"wrap", marginTop:14 }}>
            <Btn orange onClick={startTopAction} disabled={!topAction}>Start next action</Btn>
            <Btn ghost onClick={recoverLateTask} disabled={!lateTasks.length}>Recover late task</Btn>
            <Btn ghost onClick={() => setShowNewTask(true)}>Capture new task</Btn>
          </div>
        </Panel>
      )}
      ...
      {suiteTab === "reports" && (
        <div style={{ display:"grid", gridTemplateColumns:gridTwo, gap:14 }}>
          <Panel>
            <SectionTitle eyebrow="Smartsheet style" title="Management report snapshot" />
            <div style={{ color:C.creamSoft, fontSize:13, lineHeight:1.8 }}>
              <strong style={{ color:C.cream }}>Today:</strong> {todayTasks.length} active missions, {doneToday} completed.<br />
              <strong style={{ color:C.cream }}>MOPAS:</strong> {tenderTasks.length} tender tasks and {normalMopasTasks.length} normal tasks.<br />
              <strong style={{ color:C.cream }}>Risk:</strong> {lateTasks.length} late tasks, {riskDocs.length} document alerts.<br />
              <strong style={{ color:C.cream }}>Money:</strong> Month net is {rwf(moneyNet)} and ITSINDA week is {rwf(itsindaThisWeek)}.
            </div>
          </Panel>
          <Panel>
            <SectionTitle eyebrow="Boss-ready" title="What to report" action={<Btn small ghost onClick={() => goSpaceView("mopas", "daily report")}>Open daily report</Btn>} />
            <ul style={{ margin:0, paddingLeft:18, color:C.creamSoft, lineHeight:1.7, fontSize:13 }}>
              <li>Completed tasks: use Done list and daily report.</li>
              <li>Pending: use Late risk and Today missions.</li>
              <li>Tenders: use pipeline and MOPAS board.</li>
              <li>Documents: use document risk database.</li>
              <li>Tomorrow: use Command tab and End Day Review.</li>
            </ul>
          </Panel>
        </div>
      )}
      ...
      {suiteTab === "knowledge" && (
        <Panel>
          <SectionTitle eyebrow="Notion style" title="Knowledge base and reusable templates" sub="This gives you clean notes for tenders, reports, creative work, and money decisions." />
          <div style={{ display:"grid", gridTemplateColumns:gridTwo, gap:10 }}>
            {knowledgeTemplates.map(template => (
              <div key={template.title} style={{ background:C.bg, border:"1px solid "+C.border, borderRadius:14, padding:13 }}>
                <div style={{ display:"flex", justifyContent:"space-between", gap:8, marginBottom:8 }}><strong style={{ color:C.cream }}>{template.title}</strong><Badge color={C.gold}>{template.tag}</Badge></div>
                <pre style={{ margin:0, whiteSpace:"pre-wrap", color:C.creamSoft, fontFamily:"inherit", fontSize:12, lineHeight:1.5 }}>{template.body}</pre>
                <Btn small ghost onClick={() => copyText(template.body)} style={{ marginTop:10 }}>Copy template</Btn>
              </div>
            ))}
          </div>
        </Panel>
      ...
      {suiteTab === "roadmap" && (
        <Panel>
          <SectionTitle eyebrow="Linear style" title="AFTERGLOW product roadmap" sub="This is the direction from workspace to serious life/company operating system." />
          <div style={{ display:"grid", gridTemplateColumns:gridTwo, gap:10 }}>
            {roadmap.map(item => (
              <div key={item.title} style={{ background:C.bg, border:"1px solid "+item.color, borderRadius:14, padding:13 }}>
                <div style={{ display:"flex", justifyContent:"space-between", gap:8, marginBottom:8 }}><div><div style={{ color:item.color, fontSize:11, letterSpacing:2, fontWeight:900 }}>{item.phase}</div><strong style={{ color:C.cream }}>{item.title}</strong></div><Badge color={item.color}>{item.status}</Badge></div>
                <ul style={{ margin:"8px 0 0", paddingLeft:18, color:C.creamSoft, fontSize:12, lineHeight:1.6 }}>
...
function AfterglowApp() {
  ...
  const [activeSpace, setActiveSpace] = useState(() => {
    const savedSpace = readStore(ACTIVE_SPACE_KEY, "");
    const preferredSpace = savedSpace || loadAppSettings().general.defaultSpace || "wakeup";
  ...
  useEffect(() => { persistTasksNow(tasks); }, [tasks]);
  useEffect(() => {
    const persistBeforeLeave = () => persistTasksNow(tasks);
  ...
  useEffect(() => { writeStore(ACTIVE_SPACE_KEY, activeSpace); }, [activeSpace]);
  useEffect(() => { writeStore(TASK_SCOPE_FILTER_KEY, spaceFilter); }, [spaceFilter]);
  ...
  useEffect(() => {
    const method = settings?.notifications?.method || "Phone reminder + Email";
    const enabled = settings?.notifications?.phoneTaskReminders !== false && method !== "Email" && method !== "Off";
    if (!enabled || typeof window === "undefined") return undefined;
    const timeToMinutes = (time) => {
      const [h, m] = String(time || "00:00").split(":").map(Number);
      return (Number.isFinite(h) ? h : 0) * 60 + (Number.isFinite(m) ? m : 0);
    };
    const inQuietHours = (nowMinutes) => {
      const start = timeToMinutes(settings?.notifications?.quietHoursStart || "23:00");
      const end = timeToMinutes(settings?.notifications?.quietHoursEnd || "05:30");
      if (start === end) return false;
      return start > end ? (nowMinutes >= start || nowMinutes < end) : (nowMinutes >= start && nowMinutes < end);
    };
    const shouldIncludeTask = (task) => {
      if (!task || task.status === "Done") return false;
      const scope = settings?.notifications?.reminderScope || "Today + overdue";
      const today = localTodayISO();
      if (scope === "High priority only") return ["Urgent", "High"].includes(task.priority);
      if (scope === "All open tasks") return true;
    ...
    const runReminderCheck = async () => {
      try {
        ...
        const now = new Date();
        const nowMinutes = now.getHours() * 60 + now.getMinutes();
        if (inQuietHours(nowMinutes)) return;
        const today = localTodayISO();
    ...
    runReminderCheck();
    const timer = window.setInterval(runReminderCheck, 30000);
    return () => window.clearInterval(timer);
  }, [tasks, settings?.notifications?.method, settings?.notifications?.phoneTaskReminders, settings?.notifications?.reminderMinutesBefore, settings?.notifications?.reminderRepeatMinutes, settings?.notifications?.reminderScope, settings?.notifications?.quietHoursStart, settings?.notifications?.quietHoursEnd, settings?.notifications?.vibrateOnReminder]);
  ...
  const forceBackendLogout = useCallback((message = "Session expired. Please login again.") => {
    ...
    setBackendNotice({ type:"error", message });
  }, []);
  ...
  const refreshCloudTasks = useCallback(async () => {
    ...
    try {
      ...
      setTasks(prev => persistTasksNow(ensureRoutineTasksForDate(mergeCloudTasks(prev, cloudTasks), localTodayISO()).map(normalizeTask)));
      setBackendNotice({ type:"success", message:`Synced ${cloudTasks.length} cloud task${cloudTasks.length === 1 ? "" : "s"}.` });
    } catch (error) {
      if (error?.status === 401) forceBackendLogout("Session expired. Please login again.");
      else setBackendNotice({ type:"error", message:String(error?.message || error) });
    } finally {
  ...
  useEffect(() => {
    if (!auth.token || autoSyncDone || backendBusy) return;
    setAutoSyncDone(true);
    refreshCloudTasks();
  }, [auth.token, autoSyncDone, backendBusy, refreshCloudTasks]);
  ...
  const handleBackendLogin = useCallback(async (payload) => {
    ...
    try {
      ...
      if (error?.status === 401) forceBackendLogout("Invalid session. Please login again.");
      else setBackendNotice({ type:"error", message:String(error?.message || error) });
    } finally {
      setBackendBusy(false);
    }
  ...
  const uploadLocalTasksToCloud = useCallback(async () => {
    ...
    try {
      ...
      setTasks(prev => persistTasksNow(mergeCloudTasks(prev, uploaded)));
  ...
  const createTask = useCallback(async (t) => {
    ...
    setTasks(prev => {
      const next = sortTasksSmart([finalTask, ...prev]);
      persistTasksNow(next);
      return next;
    });
    setSelected(finalTask);
    setActiveSpace(finalTask.space || activeSpace);
    setSpaceFilter("all");
    setStatusFilter("All");
    ...
    try {
      ...
      if (result.data) {
        ...
        setTasks(prev => {
          const next = sortTasksSmart(prev.map(item =>
  ...
  }, [settings?.tasks?.defaultPriority, activeSpace]);
  const updateTask = useCallback(async (u) => {
    ...
    setTasks(prev => {
      ...
      const next = sortTasksSmart(mapped);
      persistTasksNow(next);
      return next;
    });
    ...
    try {
      ...
      if (result.data) {
        const cloudTask = taskFromApi(result.data);
        setTasks(prev => {
          const next = sortTasksSmart(prev.map(item => (item.id === apiPayload.id || getTaskCloudId(item) === cloudId) ? cloudTask : item));
          persistTasksNow(next);
          return next;
        });
...
This will clear ${total} existing task${total === 1 ? "" : "s"} and create today's routine tasks.`)) return;
    ...
    setSpaceFilter("all");
    setTasks(persistTasksNow(sortTasksSmart(freshTasks)));
    ...
    try {
      ...
      if (uploaded.length) setTasks(persistTasksNow(mergeCloudTasks(freshTasks, uploaded)));
      setBackendNotice({
  ...
  const generateTomorrowRoutineTasks = useCallback(() => {
    ...
    setTasks(prev => {
      ...
      const finalNext = next.length === prev.length ? prev : next;
      persistTasksNow(finalNext);
      return finalNext;
    });
  }, []);
      ...
      try {
        ...
        if (Array.isArray(data.tasks)) {
          setTasks(prev => {
            ...
            const next = ensureRoutineTasksForDate(sortTasksSmart([...byId.values()]), localTodayISO());
            persistTasksNow(next);
            return next;
          });
        }
  ...
  const clearTestData = useCallback(() => {
    ...
    setTasks(prev => {
      const next = prev.filter(t => !String(t.id || "").startsWith("TEST-") && !String(t.title || "").toLowerCase().includes("test data"));
      persistTasksNow(next);
      return next;
    });
  ...
  return (
    <div style={{ display:"flex", height:"100vh", minHeight:"100dvh", background:C.bg, color:C.cream, fontFamily:"Segoe UI, Helvetica Neue, sans-serif", overflow:"hidden", fontSize: safeSettings.appearance.fontSize === "small" ? 13 : safeSettings.appearance.fontSize === "large" ? 16 : safeSettings.appearance.compactMode ? 13 : 14 }}>
      ...
      {/* ── SIDEBAR ── */}
      <aside style={{ width: sidebarOpen ? (isMobileLayout ? Math.min(screenWidth * 0.86, 290) : 248) : 0, minWidth: sidebarOpen ? (isMobileLayout ? Math.min(screenWidth * 0.86, 290) : 248) : 0, maxWidth:"88vw", background:C.surface, borderRight:"1px solid "+C.border, display:"flex", flexDirection:"column", transition:"all .22s", overflow:"hidden", position:isMobileLayout ? "fixed" : "relative", left:0, top:0, bottom:0, height:"100dvh", zIndex:isMobileLayout ? 30 : 1, boxShadow:isMobileLayout && sidebarOpen ? "18px 0 60px #000c" : "none" }}>
        {/* Logo + identity */}
        ...
        <div style={{ flex:1, overflowY:"auto", padding:"8px 8px 4px" }}>
          ...
          {[
            ...
            return (
              <div key={item.id} onClick={() => { setView(item.id); setSelected(null); if (isMobileLayout) setSidebarOpen(false); }} style={{ padding:"8px 10px", borderRadius:8, cursor:"pointer", marginBottom:1, display:"flex", gap:8, alignItems:"center", background:active ? C.elevated : "transparent", borderLeft:"3px solid "+(active ? item.color : "transparent") }}>
                <span style={{ fontSize:13, color:active ? item.color : C.muted, width:16, textAlign:"center", flexShrink:0 }}>{item.icon}</span>
                <span style={{ flex:1, fontSize:12, fontWeight:active ? 800 : 500, color:active ? item.color : C.cream, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{item.label}</span>
              </div>
            );
          })}
        ...
        <div style={{ padding:"10px 10px 12px", borderTop:"1px solid "+C.border, display:"grid", gap:8 }}>
          {safeSettings.appearance.showCloudSyncPanel !== false && <CloudSyncPanel auth={auth} backendNotice={backendNotice} backendBusy={backendBusy} onLogin={handleBackendLogin} onRegister={handleBackendRegister} onLogout={handleBackendLogout} onRefresh={refreshCloudTasks} onUploadLocal={uploadLocalTasksToCloud} placement="top" compact />}
          <div style={{ display:"grid", gridTemplateColumns:"1fr auto auto", gap:6, alignItems:"center" }}>
            <span style={{ color:C.muted, fontSize:9, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>AFTERGLOW © 2026</span>
            ...
            <button title="Logout" onClick={handleBackendLogout} style={{ height:32, padding:"0 8px", borderRadius:8, border:"1px solid "+C.red+"55", background:"transparent", color:C.red, cursor:"pointer", fontSize:11, fontWeight:800 }}>Out</button>
          </div>
        </div>
      </aside>
      ...
      <main style={{ flex:1, display:"flex", flexDirection:"column", overflow:"hidden", minWidth:0, paddingBottom: isPhoneLayout ? 56 : 0 }}>
        <header style={{ padding:isPhoneLayout ? "9px 12px" : "12px 20px", borderBottom:"1px solid "+C.border, display:"flex", justifyContent:"space-between", alignItems:"center", background:C.surface, gap:8, flexShrink:0 }}>
          <div style={{ display:"flex", alignItems:"center", gap:8, minWidth:0 }}>
            <span onClick={() => setSidebarOpen(o => !o)} style={{ cursor:"pointer", fontSize:18, color:C.creamSoft, flexShrink:0 }}>☰</span>
            <div style={{ minWidth:0 }}>
              <div style={{ fontWeight:800, fontSize:isPhoneLayout ? 13 : 15, whiteSpace:"nowrap", overflow:"hidden", textOverflow:"ellipsis" }}>{viewTitle}</div>
              {!isPhoneLayout && !isGlobalView && <div style={{ color:C.muted, fontSize:10, marginTop:1 }}>{sp.description || ""}</div>}
            </div>
          </div>
          <div style={{ display:"flex", gap:6, alignItems:"center", flexShrink:0 }}>
            ...
            {safeSettings.appearance.showTopSettingsButton === true && !isPhoneLayout && <Btn ghost onClick={() => setView("settings")}>Settings</Btn>}
        ...
        {!isGlobalView && (
          <div style={{ padding:isPhoneLayout ? "8px 12px" : "10px 20px", borderBottom:"1px solid "+C.border, background:C.surface, display:"flex", flexWrap:"wrap", gap:8, alignItems:"center", flexShrink:0 }}>
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Search tasks, tenders, documents..." style={{ flex:"1 1 140px", minWidth:100, padding:"7px 11px", borderRadius:8, border:"1px solid "+C.border, background:C.bg, color:C.cream, outline:"none", fontSize:12, boxSizing:"border-box" }} />
            <select value={spaceFilter} onChange={e => setSpaceFilter(e.target.value)} style={{ padding:"7px 8px", borderRadius:8, border:"1px solid "+C.border, background:C.bg, color:C.cream, fontSize:12 }}>
              <option value="active">This space</option>
              <option value="all">All spaces</option>
            </select>
            <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ padding:"7px 8px", borderRadius:8, border:"1px solid "+C.border, background:C.bg, color:C.cream, fontSize:12 }}>
              {["All", ...STATUSES].map(x => <option key={x} value={x}>{x}</option>)}
            </select>
            <select value={priorityFilter} onChange={e => setPriorityFilter(e.target.value)} style={{ padding:"7px 8px", borderRadius:8, border:"1px solid "+C.border, background:C.bg, color:C.cream, fontSize:12 }}>
      ...
      {/* ── PHONE BOTTOM NAV ── */}
      {isPhoneLayout && (
        <nav style={{ position:"fixed", bottom:0, left:0, right:0, height:56, background:C.surface, borderTop:"1px solid "+C.border, display:"flex", alignItems:"stretch", zIndex:40, boxShadow:"0 -4px 20px #0006" }}>
          {mobileNavItems.map(item => {
            const active = mobileActiveId === item.id;
            return (
              <button key={item.id} onClick={item.fn} style={{ flex:1, border:"none", background:"transparent", cursor:"pointer", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", gap:2, color:item.accent ? "#fff" : active ? C.orange : C.muted, position:"relative", padding:0 }}>
                {item.accent ? (
                  <div style={{ width:34, height:34, borderRadius:"50%", background:C.orange, display:"flex", alignItems:"center", justifyContent:"center", fontSize:20, fontWeight:900, boxShadow:"0 3px 12px "+C.orange+"88" }}>{item.icon}</div>
                ) : (
                  <>
                    <span style={{ fontSize:15, lineHeight:1 }}>{item.icon}</span>
                    <span style={{ fontSize:9, fontWeight:active ? 800 : 400, lineHeight:1 }}>{item.label}</span>
                    {active && <div style={{ position:"absolute", bottom:0, left:"50%", transform:"translateX(-50%)", width:20, height:3, background:C.orange, borderRadius:"2px 2px 0 0" }} />}
                  </>
                )}
              </button>
            );
